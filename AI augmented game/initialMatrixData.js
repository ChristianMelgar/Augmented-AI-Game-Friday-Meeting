...
const initialMatrixData = {
  ...
  2: {
    pool: [
      ...
      {id: "pre_2_1", headline: "Interview-to-hypothesis mapping tool", desc: "Upload interview transcripts and get back quotes, themes, and evidence mapped directly to each hypothesis — so you know instantly which hypotheses the interviews support, which they challenge, and what\'s still unresolved."},
      ...
      {id: "pre_2_2", headline: "Self-building fact-pack HTML", desc: "A fact-pack that builds itself — as data comes in, the HTML updates automatically with charts, comparisons, and narrative. The team edits and challenges rather than builds from scratch."},
      {id: "pre_2_3", headline: "Client data validation manager", desc: "A baseline validation HTML shared with the client where they confirm, challenge, or annotate each data point directly — getting structured sign-off asynchronously rather than in a two-hour alignment meeting."},
      ...
      {id: "pre_2_4", headline: "Instant visual analysis tool", desc: "Upload the raw data and get back an instant visual analysis — outliers flagged, patterns surfaced, and the three things the data says that you didn\'t expect highlighted before the analyst has touched it."},
      {id: "pre_2_5", headline: "Analysis integrity agent", desc: "An agent that continuously cross-checks your analysis against the raw data and flags inconsistencies — so errors get caught before the client sees them, not after."},
  ...
  3: {
    pool: [
      ...
      {id: "pre_3_2", headline: "Scenario definition tool", desc: "Input your design criteria and constraints and get back a structured longlist of scenario options with the strategic logic for each — ready to pressure-test with the client before the team has built a single model."},
      {id: "pre_3_3", headline: "Auto-generated scenario one-pagers", desc: "An agent that writes a one-pager for each scenario automatically — strategic logic, key assumptions, financial summary, pros and cons — as soon as the model outputs are ready."},
      ...
      {id: "pre_3_4", headline: "Conversational recommendation builder", desc: "Define objectives and constraints in natural language and receive a recommended scenario, key trade-offs, supporting rationale, and a draft recommendation narrative."}
    ],
    placed: []
  },
  4: {
    pool: [
      ...
      //{id: "pre_4_1", headline: "Ineractive decision platform", desc: "A single environment where stakeholders review recommendations, explore scenarios, test assumptions, and capture positions before and during decision-making forums — replacing static pre-reads, presentations, and voting tools."},
     // {id: "pre_4_4", headline: "Live steering committee experience", desc: "Bring the scenario model into the steering committee itself — instead of presenting slides, the consultant facilitates a live conversation where the committee adjusts assumptions and sees the recommendation respond in real time."},
      ...
      {id: "pre_4_2", headline: "Stakeholder stress-test tool", desc: "Input the recommendation and the model and get back the three most likely challenges from the CFO, COO, and Head of Commercial — the data that addresses each, and the framing that lands best."},
